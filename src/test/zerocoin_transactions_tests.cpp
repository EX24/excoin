...
BOOST_AUTO_TEST_CASE(zerocoin_spend_test)
{
    ...
    cWallet.zexeTracker = unique_ptr<CzEXETracker>(new CzEXETracker(cWallet.strWalletFile));
    ...
    BOOST_CHECK_MESSAGE(receipt.GetStatus() == zEXE_TRX_FUNDS_PROBLEMS, "Failed Invalid Amount Check");
    ...
    BOOST_CHECK_MESSAGE(receipt2.GetStatus() == zEXE_TRX_FUNDS_PROBLEMS, "Failed Invalid Amount Check");
