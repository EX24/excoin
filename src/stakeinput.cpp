...
// The zEXE block index is the first appearance of the accumulator checksum that was used in the spend
...
{
    //The unique identifier for a zEXE is a hash of the serial
...
{
    //Create an output returning the zEXE that was staked
    CTxOut outReward;
    libzerocoin::CoinDenomination denomStaked = libzerocoin::AmountToZerocoinDenomination(this->GetValue());
    CDeterministicMint dMint;
    if (!pwallet->CreatezEXEOutPut(denomStaked, outReward, dMint))
        return error("%s: failed to create zEXE output", __func__);
    vout.emplace_back(outReward);

    //Add new staked denom to our wallet
    if (!pwallet->DatabaseMint(dMint))
        return error("%s: failed to database the staked zEXE", __func__);

    for (unsigned int i = 0; i < 3; i++) {
        CTxOut out;
        CDeterministicMint dMintReward;
        if (!pwallet->CreatezEXEOutPut(libzerocoin::CoinDenomination::ZQ_ONE, out, dMintReward))
            return error("%s: failed to create zEXE output", __func__);
...
{
    CzEXETracker* zexeTracker = pwallet->zexeTracker.get();
    CMintMeta meta;
    if (!zexeTracker->GetMetaFromStakeHash(hashSerial, meta))
        return error("%s: tracker does not have serialhash", __func__);

    zexeTracker->SetPubcoinUsed(meta.hashPubcoin, txid);
    return true;
}
...
{
    //The unique identifier for a EXE stake is the outpoint
