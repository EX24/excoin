...
CzEXETracker::CzEXETracker(std::string strWalletFile)
{
...
CzEXETracker::~CzEXETracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzEXETracker::Init()
{
...
bool CzEXETracker::Archive(CMintMeta& meta)
{
...
bool CzEXETracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzEXETracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzEXETracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzEXETracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CzEXETracker::GetSerialHashes()
{
...
CAmount CzEXETracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzEXETracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzEXETracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzEXETracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzEXETracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzEXETracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzEXETracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzEXETracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzEXETracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzEXETracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzEXETracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzEXETracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzEXETracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzEXETracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzEXETracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzEXETracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzEXETracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        for (auto& dMint : listDeterministicDB)
            Add(dMint);
        LogPrint("zero", "%s: added %d dzexe from DB\n", __func__, listDeterministicDB.size());
    }
