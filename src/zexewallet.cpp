...
CzEXEWallet::CzEXEWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zexe seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadzEXESeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.ErasezEXESeed_deprecated()) {
                    LogPrintf("%s: Updated zEXE seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zexe seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zexe wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzEXEWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WritezEXECount(nCountLastUsed);
    else if (!walletdb.ReadzEXECount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzEXEWallet::Lock()
{
    seedMaster = 0;
}

void CzEXEWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzEXEWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedTozEXE(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzEXEWallet::LoadMintPoolFromDB()
{
...
void CzEXEWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzEXEWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzEXEWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zexeTracker->HasPubcoinHash(pMint.first)) {
...
bool CzEXEWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedTozEXE(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zexeTracker which also adds to database
    pwalletMain->zexeTracker->Add(dMint, true);
...
void CzEXEWallet::SeedTozEXE(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzEXEWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzEXEWallet::GenerateDeterministiczEXE(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzEXEWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzEXEWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
