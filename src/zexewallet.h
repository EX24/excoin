...
#ifndef EXE_zEXEWALLET_H
#define EXE_zEXEWALLET_H
...
public:
    CzEXEWallet(std::string strWalletFile);
    ...
    void GenerateDeterministiczEXE(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly = false);
    ...
    void UpdateCount();
    void Lock();
    void SeedTozEXE(const uint512& seed, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key);

private:
    uint512 GetZerocoinSeed(uint32_t n);
};

#endif //EXE_zEXEWALLET_H
