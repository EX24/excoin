...
public:
    ...
    bool WritezEXESeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadzEXESeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadzEXESeed_deprecated(uint256& seed);
    bool ErasezEXESeed();
    bool ErasezEXESeed_deprecated();

    bool WritezEXECount(const uint32_t& nCount);
    bool ReadzEXECount(uint32_t& nCount);
